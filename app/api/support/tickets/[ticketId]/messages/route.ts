...
export async function POST(req: Request, context: RouteContext) {
  try {
    ...
    if (!message) {
      return NextResponse.json({ error: "請填寫訊息內容。", build_tag: FORMAL_OPS_BUILD_TAG }, { status: 400 });
    }
    ...
    if (ticket.data.status === "closed") {
      return NextResponse.json({ error: "這張客服單已關閉，請重新開啟或建立新的客服單。", build_tag: FORMAL_OPS_BUILD_TAG }, { status: 400 });
    }
    ...
    if (inserted.error || !inserted.data) {
      return NextResponse.json({ error: inserted.error?.message || "新增訊息失敗。", build_tag: FORMAL_OPS_BUILD_TAG }, { status: 400 });
    }

    await supabaseAdmin
      .from("support_tickets")
      .update({
        status: ticket.data.status === "resolved" ? "open" : ticket.data.status,
    ...
    if (error?.message === "UNAUTHORIZED") {
      return NextResponse.json({ error: "請先登入後再回覆客服單。", build_tag: FORMAL_OPS_BUILD_TAG }, { status: 401 });
    }
    return NextResponse.json({ error: error?.message || "新增客服訊息失敗。", build_tag: FORMAL_OPS_BUILD_TAG }, { status: 500 });
  }
}
