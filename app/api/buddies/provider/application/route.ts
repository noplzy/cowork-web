...
export async function GET(req: Request) {
  try {
    ...
    if (error?.message === "UNAUTHORIZED") return NextResponse.json({ error: "請先登入後再查看申請。", build_tag: BUDDIES_BUILD_TAG }, { status: 401 });
...
export async function POST(req: Request) {
  try {
    ...
    if (error?.message === "UNAUTHORIZED") return NextResponse.json({ error: "請先登入後再送出申請。", build_tag: BUDDIES_BUILD_TAG }, { status: 401 });
