...
export async function GET(req: Request) {
  try {
    ...
    const [
      ...
      supabaseAdmin.from("billing_ledger")
      ...
      supabaseAdmin.from("refund_requests")
      ...
      supabaseAdmin.from("ecpay_invoice_tasks")
        ...
        .eq("user_id", userId).order("created_at", { ascending: false }).limit(limit),
      supabaseAdmin.from("ecpay_refund_tasks")
      ...
      supabaseAdmin.from("subscription_profiles")
        .select("id,provider,plan_code,status,period_amount,period_type,frequency,next_charge_at,current_period_start,current_period_end,cancel_requested_at,cancelled_at,last_provider_error,invoice_preference,commercial_entitlement_status,entitlement_applied_at,created_at,updated_at")
        .eq("user_id", userId).order("created_at", { ascending: false }).limit(limit),
      supabaseAdmin.from("subscription_events")
        ...
        .eq("user_id", userId).order("created_at", { ascending: false }).limit(limit),
      supabaseAdmin.from("entitlement_events")
        .select("id,event_type,plan_code,entitlement_key,quantity,valid_from,valid_until,payment_order_id,metadata,created_at")
        .eq("user_id", userId).order("created_at", { ascending: false }).limit(limit),
      supabaseAdmin.from("user_usage_wallet_events")
        .select("id,wallet_id,event_type,resource_key,delta_quantity,overage_delta,balance_after,payment_order_id,room_id,access_session_id,created_at")
        .eq("user_id", userId).order("created_at", { ascending: false }).limit(limit),
    ]);

    const firstError = [
      orders.error, ledger.error, refunds.error, invoices.error,
      invoiceTasks.error, refundTasks.error, subscriptions.error,
      subscriptionEvents.error, entitlementEvents.error, walletEvents.error,
    ].find(Boolean);
    ...
    return NextResponse.json({
      ...
      wallet_events: walletEvents.data || [],
      build_tag: P2_BUILD_TAGS.entitlement,
    });
  } catch (error) {
    const message = error instanceof Error ? error.message : "讀取帳務資料失敗。";
    return NextResponse.json(
