...
type PatchBody = {
  status?: "open" | "triaged" | "actioned" | "dismissed" | "closed";
  admin_note?: string;
  create_case?: boolean;
};
...
export async function PATCH(req: Request, context: RouteContext) {
  try {
    ...
    if (report.error || !report.data) {
      return NextResponse.json({ error: report.error?.message || "找不到檢舉。", build_tag: ADMIN_OPS_BUILD_TAG }, { status: 404 });
    }

    let linkedCaseId = report.data.linked_moderation_case_id ?? null;

    if (body.create_case && !linkedCaseId) {
      const newCase = await supabaseAdmin
        ...
        .insert({
          ...
          metadata: { category: report.data.category },
        })
        .select("*")
        .single();

      if (newCase.error || !newCase.data) {
        return NextResponse.json({ error: newCase.error?.message || "建立 moderation case 失敗。", build_tag: ADMIN_OPS_BUILD_TAG }, { status: 400 });
      }
    ...
    if (body.admin_note) patch.admin_note = cleanText(body.admin_note, 6000);
    ...
    if (updated.error || !updated.data) {
      return NextResponse.json({ error: updated.error?.message || "更新檢舉失敗。", build_tag: ADMIN_OPS_BUILD_TAG }, { status: 400 });
    }

    await writeAdminAudit(req, {
      adminUserId: admin.userId,
      actionType: "admin_user_report_updated",
      targetType: "user_report",
      targetId: reportId,
      metadata: { status: body.status, linked_case_id: linkedCaseId },
    });
