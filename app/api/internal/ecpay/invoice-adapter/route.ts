...
const BUILD_TAG = "ecpay-invoice-adapter-v122-2026-07-02";
...
async function resolveOriginalInvoiceDate(payload: AdapterPayload) {
  ...
  let eventQuery = supabaseAdmin
    .from("invoice_events")
    .select("id, invoice_number, issued_at, created_at, metadata")
  ...
  let taskQuery = supabaseAdmin
    ...
    .select("id, provider_invoice_no, provider_payload, processed_at, updated_at, created_at")
    .eq("payment_order_id", paymentOrderId)
...
async function handleVoid(payload: AdapterPayload, invoiceDate?: string, invoiceDateSource?: string) {
  ...
  const result = await invalidB2CInvoice({ invoiceNo: firstString(followup.original_invoice_number, payload.invoice_event?.invoice_number), invoiceDate: resolved.invoiceDate, reason: firstString(followup.reason, "退款作廢") });
