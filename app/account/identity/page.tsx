...
function IdentityContent() {
  ...
  const canVerify = Boolean(normalizedPhone && otp.trim()) && !sending && !verifying;
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setPhoneInput(p.startsWith("+8869") ? `0${p.slice(4)}` : p);

      const pendingPhoneChange = Boolean(
        (data.user as any)?.new_phone || (data.user as any)?.phone_change_sent_at
      );

      setHasSent(pendingPhoneChange);
      setConfirmed(Boolean(p && (data.user as any).phone_confirmed_at));
    })();
  ...
  return (
    <Image20SidebarShell title="身份驗證" email={email}>
      <div className="i20-page" data-image20-dom-page="identity-v7">
        ...
        <section className="i20-room-layout" style={{ marginTop: 18 }}>
          <article className="i20-panel">
            ...
            <div className="i20-list" style={{ marginTop: 18 }}>
              ...
              {!hasSent && otp.trim() ? (
                <div className="i20-card">若你已收到簡訊但按鈕先前無法點擊，現在可直接完成驗證。</div>
              ) : null}
