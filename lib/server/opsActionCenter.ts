...
export const OPS_ACTION_CENTER_BUILD_TAG = "ops-action-center-v111-2026-06-04";
...
export async function buildOpsActionCenter() {
  ...
  const detected: DetectedAction[] = [
    ...support.data.map((row: any) => item({
      key: `support:${row.id}`,
      source_type: "support_ticket",
      source_id: row.id,
      title: `客服單待處理：${row.subject || row.id}`,
      description: `${row.category || "support"}｜${row.status}｜${row.priority || "normal"}`,
      category: "support",
      severity: row.priority === "urgent" ? "urgent" : row.priority === "high" ? "high" : "normal",
      href: `/admin/support/${row.id}`,
      created_at: row.updated_at || row.created_at,
    })),
    ...refunds.data.map((row: any) => item({
      key: `refund:${row.id}`,
      source_type: "refund_request",
      source_id: row.id,
      title: `退款待處理：${row.status}`,
      description: `${row.reason_category || "refund"}｜NT$${row.amount_twd ?? "—"}`,
      category: "refund",
      severity: ["approved", "processing"].includes(row.status) ? "high" : "normal",
      href: "/admin/refunds",
      created_at: row.created_at,
    })),
    ...reports.data.map((row: any) => item({
      key: `report:${row.id}`,
      source_type: "user_report",
      source_id: row.id,
      title: `安全檢舉待處理：${row.category || "report"}`,
      description: `${row.target_type || "target"}｜${row.status}`,
      category: "safety",
      severity: row.severity === "critical" ? "critical" : row.severity === "high" ? "high" : "normal",
      href: "/admin/safety",
      created_at: row.created_at,
    })),
    ...moderation.data.map((row: any) => item({
      key: `moderation:${row.id}`,
      source_type: "moderation_case",
      source_id: row.id,
      title: `治理案件待處理：${row.target_type || "case"}`,
      description: `${row.status}｜${row.severity || "normal"}`,
      category: "moderation",
      severity: row.severity === "critical" ? "critical" : row.severity === "high" ? "high" : "normal",
      href: `/admin/moderation/${row.id}`,
      created_at: row.updated_at || row.created_at,
    })),
    ...invoiceTasks.data.map((row: any) => item({
      key: `invoice_task:${row.id}`,
      source_type: "ecpay_invoice_task",
      source_id: row.id,
      title: `發票任務：${row.status}`,
      description: row.last_error || "invoice task needs processing",
      category: "billing",
      severity: row.status === "failed" ? "high" : "normal",
      href: "/admin/billing/automation",
      created_at: row.created_at,
    })),
    ...refundTasks.data.map((row: any) => item({
      key: `refund_task:${row.id}`,
      source_type: "ecpay_refund_task",
      source_id: row.id,
      title: `退款任務：${row.status}`,
      description: row.last_error || "refund task needs processing",
      category: "billing",
      severity: row.status === "failed" ? "high" : "normal",
      href: "/admin/billing/automation",
      created_at: row.created_at,
    })),
    ...subscriptionTasks.data.map((row: any) => item({
      key: `subscription_task:${row.id}`,
      source_type: "ecpay_subscription_task",
      source_id: row.id,
      title: `訂閱任務：${row.action_type || "task"} / ${row.status}`,
      description: row.last_error || "subscription task needs processing",
      category: "subscription",
      severity: row.status === "failed" ? "high" : "normal",
      href: "/admin/billing/automation",
      created_at: row.created_at,
    })),
    ...notifications.data.map((row: any) => item({
      key: `notification:${row.id}`,
      source_type: "notification_outbox",
      source_id: row.id,
      title: `通知待處理：${row.channel} / ${row.status}`,
      description: row.last_error || row.subject || row.template_key,
      category: "notification",
      severity: row.priority === "urgent" ? "urgent" : row.status === "failed" ? "high" : "normal",
      href: "/admin/notifications",
      created_at: row.created_at,
    })),
    ...ghostRooms.data.map((row: any) => item({
      key: `ghost_room:${row.id}`,
      source_type: "room",
      source_id: row.id,
      title: `可能逾期未清理房間：${row.title || row.id}`,
      description: `scheduled_end_at ${row.scheduled_end_at || "—"}`,
      category: "rooms",
      severity: "high",
      href: `/admin/rooms/${row.id}`,
      created_at: row.scheduled_end_at || row.created_at,
    })),
  ];

  const errors = [support, refunds, reports, moderation, invoiceTasks, refundTasks, subscriptionTasks, notifications, ghostRooms, manualItems]
    .map((x) => x.error)
    .filter(Boolean);
