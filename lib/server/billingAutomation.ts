...
export const BILLING_AUTOMATION_BUILD_TAG = "billing-automation-v122-2026-07-02";
...
async function postJson(endpoint: string, payload: Record<string, unknown>) {
  ...
  const response = await fetch(endpoint, {
    method: "POST",
    headers: { "Content-Type": "application/json", "x-calmco-billing-build": BILLING_AUTOMATION_BUILD_TAG, "x-internal-secret": adapterSecret },
...
function extractInvoiceNumber(providerResult: any) {
  return providerResult?.invoice_number ?? providerResult?.InvoiceNo ?? providerResult?.InvoiceNumber ?? providerResult?.invoiceNo ?? providerResult?.provider_result?.data?.InvoiceNo ?? null;
}

function extractInvoiceRandomNumber(providerResult: any) {
  return providerResult?.random_number ?? providerResult?.RandomNumber ?? providerResult?.RandomNum ?? providerResult?.RtnRandomNumber ?? providerResult?.provider_result?.data?.RandomNumber ?? null;
}
...
async function resolveOriginalInvoiceContext(invoice: any) {
  const paymentOrderId = invoice?.payment_order_id || "";
  const invoiceNumber = firstString(invoice?.invoice_number, invoice?.metadata?.invoice_number);
  const randomNumber = firstString(invoice?.invoice_random_number, invoice?.metadata?.invoice_random_number);
  const directDate = normalizeDateOnly(invoice?.metadata?.invoice_date || invoice?.metadata?.original_invoice_date || invoice?.metadata?.issued_at || invoice?.metadata?.provider_result?.issued_at || invoice?.metadata?.provider_result?.IssueDate || invoice?.metadata?.provider_result?.provider_result?.data?.InvoiceDate || invoice?.metadata?.provider_result?.data?.InvoiceDate || invoice?.issued_at);
  if (directDate) return { invoiceNumber, randomNumber, invoiceDate: directDate, source: "followup_invoice_event", issuedInvoiceEventId: null, issuedInvoiceTaskId: null };
  ...
  if (paymentOrderId) {
    let issuedQuery = supabaseAdmin
      ...
      .limit(1);
    if (invoiceNumber) issuedQuery = issuedQuery.eq("invoice_number", invoiceNumber);
    ...
    if (issued.data) {
      ...
      if (date) return { invoiceNumber: firstString(invoiceNumber, issued.data.invoice_number), randomNumber: firstString(randomNumber, issued.data.invoice_random_number), invoiceDate: date, source: "issued_invoice_event", issuedInvoiceEventId: issued.data.id, issuedInvoiceTaskId: null };
    }
    ...
    let issueTaskQuery = supabaseAdmin
      ...
      .order("created_at", { ascending: false })
      .limit(1);
    if (invoiceNumber) issueTaskQuery = issueTaskQuery.eq("provider_invoice_no", invoiceNumber);
    ...
    if (issueTask.data) {
      ...
      if (date) return { invoiceNumber: firstString(invoiceNumber, issueTask.data.provider_invoice_no), randomNumber: firstString(randomNumber, issueTask.data.provider_random_number), invoiceDate: date, source: "issued_invoice_task", issuedInvoiceEventId: null, issuedInvoiceTaskId: issueTask.data.id };
    }
  }

  return { invoiceNumber, randomNumber, invoiceDate: "", source: "missing_original_invoice_date", issuedInvoiceEventId: null, issuedInvoiceTaskId: null };
}

async function buildInvoiceAdapterPayload(task: any, invoice: any, actionType: string) {
  const order = invoice?.payment_orders || {};
  const common = {
    contract_version: "calmco-ecpay-invoice-adapter-v3",
  ...
  const originalInvoice = await resolveOriginalInvoiceContext(invoice);
  return {
    ...common,
...
async function recordInvoiceFailedEvent(invoice: any, task: any, message: string) {
  await supabaseAdmin.from("invoice_events").insert({ user_id: invoice.user_id, payment_order_id: invoice.payment_order_id, provider: "ecpay_invoice", event_type: "failed", invoice_number: invoice.invoice_number ?? null, invoice_random_number: invoice.invoice_random_number ?? null, metadata: { task_id: task.id, action_type: task.action_type || invoiceActionFromEvent(invoice), message, build_tag: BILLING_AUTOMATION_BUILD_TAG } });
}
...
export async function processInvoiceTasks(limit = 20) {
  ...
  for (const invoice of pending.data ?? []) {
    ...
    try {
      const adapterPayload = await buildInvoiceAdapterPayload(task, invoice, actionType);
      ...
      if (actionType === "issue") {
        ...
        await supabaseAdmin.from("ecpay_invoice_tasks").update({ status: "issued", provider_invoice_no: invoiceNumber, provider_random_number: randomNumber, provider_payload: providerResult, last_error: null, processed_at: new Date().toISOString(), updated_at: new Date().toISOString() }).eq("id", task.id);
        ...
        if (requestedManual) {
          ...
          await markInvoiceTaskManual(task, reason, providerResult);
          await supabaseAdmin.from("invoice_events").insert({ user_id: invoice.user_id, payment_order_id: invoice.payment_order_id, provider: "ecpay_invoice", event_type: "manual_required", invoice_number: invoice.invoice_number || null, invoice_random_number: invoice.invoice_random_number || null, metadata: { task_id: task.id, source_invoice_event_id: invoice.id, action_type: actionType, refund_request_id: task.refund_request_id ?? invoice.metadata?.refund_request_id ?? null, provider_result: providerResult, build_tag: BILLING_AUTOMATION_BUILD_TAG } });
        ...
        await supabaseAdmin.from("ecpay_invoice_tasks").update({ status: followupEvent === "voided" || followupEvent === "allowance_issued" ? followupEvent : "completed", provider_invoice_no: invoiceNumber, provider_random_number: randomNumber, provider_task_id: providerResult.task_id ?? providerResult.TradeNo ?? providerResult.allowance_number ?? null, provider_payload: providerResult, last_error: null, processed_at: new Date().toISOString(), updated_at: new Date().toISOString() }).eq("id", task.id);
      ...
      await supabaseAdmin.from("ecpay_invoice_tasks").update({ status: "failed", last_error: message, updated_at: new Date().toISOString() }).eq("id", task.id);
...
async function requestInvoiceVoidOrAllowanceIfNeeded(refund: any, providerResult: any) {
  ...
  const inserted = await supabaseAdmin.from("invoice_events").insert({ user_id: refund.user_id, payment_order_id: refund.payment_order_id, provider: "ecpay_invoice", event_type: "void_or_allowance_required", invoice_number: issued.data.invoice_number, invoice_random_number: issued.data.invoice_random_number, issued_at: issued.data.issued_at ?? null, metadata: { refund_request_id: refund.id, refund_amount_twd: refund.amount_twd, invoice_date: invoiceDate || null, original_invoice_date: invoiceDate || null, issued_invoice_event_id: issued.data.id, provider_result: providerResult, note: "退款已完成，需依開票日與退款情境人工確認作廢或折讓。", build_tag: BILLING_AUTOMATION_BUILD_TAG } }).select("id").single();
...
export async function processRefundTasks(limit = 20) {
  ...
  for (const refund of pending.data ?? []) {
    ...
    if (!liveEnabled || !endpoint) {
      await supabaseAdmin.from("ecpay_refund_tasks").update({ status: "manual_required", last_error: "ECPAY_REFUND_API_ENABLED / ECPAY_REFUND_ENDPOINT not enabled", updated_at: new Date().toISOString() }).eq("id", task.id);
